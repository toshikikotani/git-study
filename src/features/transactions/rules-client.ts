 ...
 *
 * `DEFAULT_DETECTION_RULES`(FR-21 の固定3件)だけでは、DB に保存済みの
 * 学習ルール(classification_rules, is_learned=true。`/rules` の「AIにルール
 * 相談」から作られたものなど)が次回以降の取り込みに反映されない。ここで
 * DB 保存分とカテゴリ名を取得し、呼び出し側で `DEFAULT_DETECTION_RULES` と
 * 合わせて `buildPreview()` に渡す(カテゴリ名も一緒に返すのは、ルールが
 * categoryId を設定してもプレビュー側は id しか知らず「未分類」のまま
 * 表示されてしまうため)。
 */
