...
| M2-7b | `ImapMailSource` の実装(imapflow で `MailSource` を満たす) | S | M2-7 |
| M2-7c | `/api/cron/import-gmail` と GitHub Actions ワークフロー(毎朝取得) | S | M2-7b, M0-3 |
...
| B-4 | Supabase / Vercel / Anthropic のアカウントと API キー | **本人によるアカウント作成**。M0-2 の着手前に必要 |
...
1. **アカウント作成**(B-4):Supabase / Vercel / Anthropic Console。M0-2 以降がここで止まる
2. **Discord Webhook**(B-3):サーバーとチャンネルを1つ作り、Webhook URL を発行
3. **負債の棚卸し**(B-1):借入先ごとの残高・金利・最低返済額・返済日。**これが本システムの目的そのもの**なので、M1-2(債務の登録画面)ができたらそこに直接入力するのが早い
